...
char _inode_bitmap[1024];		// the inode bitmap array 128 will work
...
/*
/****************************************************************************/
//Mounting a disk involves making a storage device accessible to the operating system and its users
void mountSFS()
{
	int i;
	char buffer[1024];

	df = fopen("sfs.disk", "r+b");// Open a binary file for both reading and writing also rb+
	if (df == NULL)
	{
	...
	BLB = stoi(buffer, 3); // total no of blocks
	INB = stoi(buffer + 3, 3); // total no of entries of entries in inode table 
	...
	for (i = 0; i <=BLB; i++)
		free_disk_blocks -= (_block_bitmap[i] - '0');
...
int writeSFS(int block_number, char buffer[1024])
{
	...
	fseek(df, block_number * 1024, SEEK_SET);//0  // set file pointer at right position
...
void ls()
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		for (j = 0; j < 4; j++)
		{
			...
			{ // entry is for a directory; print it in RED
...
void stats()
{
	...
	for (i = 0; i <= BLB; i++)
		blocks_free -= (_block_bitmap[i] - '0');
	for (i = 0; i <= INB; i++)
		inodes_free -= (_inode_bitmap[i] - '0');
...
void display(char *fname)
{
	...
	{ // display file contents to standard output
	  // enter your code below
		...
		{
			if (blocks[i] == 0)
				continue; // 0 means pointing to nothing
			char outBuffer[1024];

			readSFS(blocks[i], (char *)outBuffer);
			printf("%s", outBuffer);
		}
		printf("\n");
	}
...
void create(char *fname)
{
	...
	{ // otherwise
		...
		strncpy(_inode_table[empty_ientry].XX, "00", 2); // file is just created; so no blocks assigned to it yet
		strncpy(_inode_table[empty_ientry].YY, "00", 2);
		strncpy(_inode_table[empty_ientry].ZZ, "00", 2);

		char data[3 * BLOCK_SIZE];
		int size;
		if ((size = take_user_input(data)) == -1)
		{
			printf("Error: Input too long. File has been created but not written to.\n");
			return;
		}
		...
		{
			int block2 = getBlock();
			writeSFS(block2, data + BLOCK_SIZE);
		...
		{
			int block3 = getBlock();
			writeSFS(block3, data + 2 * BLOCK_SIZE);
...
void rmdir(int d_inode)
{
	...
	// read inode entry for current directory
	// in SFS, an inode can point to three blocks at the most
	blocks[0] = stoi(_inode_table[d_inode].XX, 2);
	blocks[1] = stoi(_inode_table[d_inode].YY, 2);
	blocks[2] = stoi(_inode_table[d_inode].ZZ, 2);

	// its a directory; so the following should never happen
	if (_inode_table[d_inode].TT[0] != 'D')
	{
...
void rm(char *name)
{
	...
	if (!found)
	{
		printf("%.252s: No such fil or directory.\n", name);
	}
	else
	{
		if (_inode_table[e_inode].TT[0] == 'D')
	...
	writeSFS(blocks[i], (char *)_directory_entries);   // now write this block back to the disk
	writeSFS(BLOCK_INODE_TABLE, (char *)_inode_table); // phew!! write the inode table back to the disk
}
